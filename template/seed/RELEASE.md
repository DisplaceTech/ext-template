...
- `Cargo.toml` (`[package].version`). The cargo build script consumes
  ...
  may in a future `Displace\{{NAMESPACE}}\VERSION` constant.
...
with the tag-derived version Composer {{NAME}}s. The `branch-alias` under
...
make stubs && git diff stubs/{{NAME}}.stubs.php
```
...
4. Stages `{{NAME}}.so` / `{{NAME}}.dylib` in the right shape.
5. Tarballs it as
   `php_{{NAME}}-{version}_php{minor}-{arch}-{os}[-{libc}].tar.gz`
...
1. Visit https://github.com/DisplaceTech/ext-{{NAME}}/releases.
...
by default. The first time you ship `ext-{{NAME}}`, register the package:

1. Go to <https://packagist.org/login/> and sign in with GitHub.
2. Click **Submit** in the top nav.
3. Paste `https://github.com/DisplaceTech/ext-{{NAME}}` into the repo
   URL field and submit.
4. Packagist reads `composer.json`, validates the `type: php-ext`
   block, and registers the package as `displace/ext-{{NAME}}`.
...
- **Per-repo webhook.** If you don't want the account-wide hook:
  GitHub → ext-{{NAME}} settings → Webhooks → Add webhook. URL is
...
> The displace/ext-{{NAME}} package of which you are a maintainer had
...
curl -XPOST -H 'content-type:application/json' \
  ...
  -d '{"repository":{"url":"https://github.com/DisplaceTech/ext-{{NAME}}"}}'
```
...
`php_{{NAME}}-{version}_php{minor}-{arch}-{os}-{libc}.tar.gz` for the
caller's environment, extract `{{NAME}}.{so,dylib}`, and drop it into the
...
- **ZTS PHP** is enabled in `composer.json` (`support-zts: true`) and
  ...
  read-only; every `transcribe()` builds and drops its own {{NAME}}
...
| One matrix leg fails to compile {{NAME}}.cpp            | Check the runner's cmake version — bump system-deps install if needed. |
| PIE can't find a matching binary                     | Verify the tarball filename — PIE matches verbatim on arch/os/libc. |
| `php -m` doesn't show `{{NAME}}` after `pie install`    | Re-run PIE with `-v` to see where it dropped the artifact and which `php.ini` it added to. |
